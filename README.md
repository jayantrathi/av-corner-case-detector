...
Self-driving perception models are trained on a fixed set of classes — car, person, road, sign. Anything outside that set gets confidently mislabeled instead of flagged, which is a problem when the thing outside the set is a tire in your lane.

This is an anomaly detector for that gap. A segmentation network is trained to become *unconfident* on unfamiliar objects, so wherever it's unsure on the road, that's a likely hazard. A depth-based filter then removes false alarms from flat road markings and manholes. The whole model was trained from scratch on an Apple Silicon GPU (M4 Pro, MPS) — no cloud, no pretrained anomaly checkpoint.
...
```bash
# train the segmenter (Apple Silicon)
